...
function Profile() {
  ...
  const handleSave = async () => {
    try {
      await apiService.updateStudent(student._id, editableStudent);
  ...
  useEffect(() => {
    const fetchRecommendations = async () => {
      ...
      try {
        ...
        const result = await mlService.getRecommendations(requestData);
        if (result.success) {
          setRecommendations(result.data.eligible_colleges);
        } else {
                ...
                <div key={index} className="pie-chart-container">
                  ...
                  <PieChart width={450} height={500}>
                    <Pie
                      ...
                      cy="50%"
                      outerRadius={100}
                      label={({ name, value }) => <span style={{ color: 'black', fontWeight: 'bold' }}>{`${name}: ${value}%`}</span>}
                      labelLine={true}
                    >
                      {COLORS.map((color, i) => (
                        <Cell
